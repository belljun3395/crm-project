...
import type { PostTemplateRequest } from '../types/EmailRequest';
// 이메일 도메인 API를 import
...
async function saveTemplate() {
  try {
    ...
    if (isEditing.value) {
      ...
      await emailApi.createEmailTemplate(dataToSave); // OpenAPI에 update(수정) 엔드포인트 없음. 필요시 구현
    } else {
      await emailApi.createEmailTemplate(dataToSave);
    }
