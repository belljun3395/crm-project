...
async function fetchUsers() {
  ...
  try {
    ...
    // 사용자 목록을 불러옵니다.
    const response = await userApi.getUsers(); // TODO: Pagination 지원 시 파라미터 추가
    users.value = response.users || [];
    // 사용자 수는 별도 API로 관리
    const countRes = await userApi.getTotalUserCount();
    totalCount.value = countRes.totalCount || 0;
