...
import api from '../api';
import type { TemplateWithHistoryDto } from '../types/Email';
...
import type { TemplateDto } from '../types/Email';
...
async function fetchEmailTemplates() {
  ...
  try {
    ...
    const response = await api.getEmailTemplates(); // TODO: Pagination 지원 시 파라미터 추가
...
async function deleteTemplate(templateId: number) {
  if (confirm('정말로 이 템플릿을 삭제하시겠습니까?')) {
    try {
      ...
      const res = await api.deleteEmailTemplate(templateId, false);
      if (res.success) {
        alert('템플릿이 삭제되었습니다.');
