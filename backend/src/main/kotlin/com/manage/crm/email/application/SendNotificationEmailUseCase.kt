...
class SendNotificationEmailUseCase(
    ...
    suspend fun execute(useCaseIn: SendNotificationEmailUseCaseIn): SendNotificationEmailUseCaseOut {
        ...
        val notificationProperties = getEmailNotificationProperties(templateVersion, templateId).apply {
            campaign?.let {
        ...
        generateNotificationDto(targetUsers, notificationProperties, campaign?.id)
    ...
    private suspend fun generateNotificationDto(targetUsers: Map<Email, User>, notificationProperties: NotificationEmailTemplatePropertiesModel, campaignId: Long?): List<SendEmailInDto> {
        val emailContentPairList = mutableListOf<Pair<Email, Content>>()
        targetUsers.forEach { (email, user) ->
            val content = emailContentService.genUserEmailContent(user, notificationProperties, campaignId)
