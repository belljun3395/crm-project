...
class CampaignCacheManager(
    ...
    fun loadHash(hash: Map<String, Any>): Campaign {
        val properties =
            hash
                .filterKeys { it.startsWith(CAMPAIGN_PROPERTIES_KEY_PREFIX) }
                .mapKeys { it.key.removePrefix(CAMPAIGN_PROPERTIES_KEY_PREFIX) }
                .mapValues { it.value.toString() }
        val campaignPropertiesMap =
    ...
    private suspend fun saveWithName(campaign: Campaign) {
        redisTemplate
            .opsForValue()
            .set("${CAMPAIGN_CACHE_KEY_PREFIX}${Campaign.UniqueFields.NAME}${SPLIT}${campaign.name}", campaign.id!!)
            .awaitSingle()
    }
    ...
    suspend fun load(id: Long): Campaign? {
        ...
        val keys =
            redisTemplate
                .opsForHash<String, Any>()
                .keys("${CAMPAIGN_CACHE_KEY_PREFIX}$id")
                .collectList()
                .awaitSingle()
        for (prop in keys) {
            redisTemplate
     ...
     */
    suspend fun loadAndSaveIfMiss(
        id: Long,
        findForLoad: suspend () -> Campaign?,
    ): Campaign? =
        runCatching { load(id) }
    ...
    suspend fun load(
        ...
        redisTemplate
            .opsForValue()
            .get("${CAMPAIGN_CACHE_KEY_PREFIX}$uniqKey${SPLIT}$value")
            .awaitSingleOrNull()
            ?.toString()
            ?.toLong()
            ?.let { load(it) }
    ...
    suspend fun loadAndSaveIfMiss(
        ...
        runCatching { load(uniqKey, value) }
    ...
    suspend fun evict(
        ...
        redisTemplate.delete("${CAMPAIGN_CACHE_KEY_PREFIX}${Campaign.UniqueFields.NAME}${SPLIT}$campaignName").awaitSingleOrNull()
    }
}
