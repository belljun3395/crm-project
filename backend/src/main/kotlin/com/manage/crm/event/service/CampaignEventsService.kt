package com.manage.crm.event.service

import com.manage.crm.event.domain.Event
import org.springframework.stereotype.Service

@Service
class CampaignEventsService(
    private val campaignEventsRepository: com.manage.crm.event.domain.repository.CampaignEventsRepository,
    private val eventsRepository: com.manage.crm.event.domain.repository.EventRepository
) {
    suspend fun findAllEventsByCampaignId(campaignId: Long): List<Event> {
        val eventIds = campaignEventsRepository.findAllByCampaignId(campaignId).map { it.eventId }
        return eventsRepository.findAllByIdIn(eventIds)
    }

    suspend fun getAllEventUserIdsByCampaignId(campaignId: Long): Set<Long> {
        return this.findAllEventsByCampaignId(campaignId).map { it.userId }.toSet()
    }
}
