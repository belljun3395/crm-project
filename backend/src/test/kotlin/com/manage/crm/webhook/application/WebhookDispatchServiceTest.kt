        ...
        given("dispatching webhook events") {
            ...
            `when`("active webhooks exist for the event") {
                then("send payload to each active webhook and persist delivery logs") {
                    ...
                    coEvery { webhookRepository.findActiveByEvent(WebhookEventType.USER_CREATED.value) } returns
                        listOf(
                            webhook1,
                            webhook2,
                        )
                    coEvery { webhookClient.send(any(), any()) } returns
                    ...
                    payloadSlot.captured.eventId
                        .isNotBlank()
                        .shouldBeTrue()
                    payloadSlot.captured.occurredAt
                        .isNotBlank()
                        .shouldBeTrue()
