    ...
    BehaviorSpec({
        ...
        given("SendNotificationEmailUseCase") {
            ...
            `when`("send notification but not found by email template id and template version") {
                ...
                then("should throw exception") {
                    ...
                    exception.message shouldBe
                        "EmailTemplate not found by templateId and version: ${useCaseIn.templateId}, ${useCaseIn.templateVersion}"
                }
                    ...
                    EmailTemplate.new(
                        ...
                        body =
                            "<html><head></head><body>" +
                                "<a th:text=\"\${user_email}\"></a>" +
                                "<a th:text=\"\${user_name}\"></a>" +
                                "<a th:text=\"\${campaign_eventCount}\"></a>" +
                                "</body></html>",
                        variables =
                            Variables(
                                listOf(
            ...
            `when`("segmentId is provided") {
                ...
                coEvery { mailService.send(any(SendEmailInDto::class)) } returns
                    SendEmailOutDto(
                        userId = 1L,
                        emailBody = "emailBody",
                        messageId = "messageId",
