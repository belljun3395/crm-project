 ...
 */
class BrowseEmailSendHistoriesUseCasePaginationTest :
    BehaviorSpec({
        ...
        fun emailSendHistoryStubs(size: Int) =
            (1..size).map { idx ->
                EmailSendHistoryFixtures
                    .giveMeOne()
                    .withId(idx.toLong())
        ...
        given("BrowseEmailSendHistoriesUseCase with pagination") {
            ...
            `when`("page size exceeds maximum limit (100)") {
                ...
                val result =
                    useCase.execute(
                        BrowseEmailSendHistoriesUseCaseIn(userId = null, sendStatus = null, page = 0, size = requestedSize),
                    )
